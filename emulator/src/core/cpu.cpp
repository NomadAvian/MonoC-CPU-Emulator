...
void CPU::Ecall() {
    ...
    switch (static_cast<ecall>(ecall_number)) {
        ...
        case ecall::kPrintint: {
            Word value = x[10].value;
            std::cout << static_cast<int32_t>(value);
            halted_ = false;
            break;
        }
        case ecall::kPrintstring:{
            Word addr = x[10].value;
            while(true) {
                Byte byte = ram_.ReadByte(addr);
                if(byte == 0) {
                    halted_ = false;
                    return;
                }
        ...
        case ecall::kReadint:{
            int32_t value;
            std::cin >> value;
            WriteReg(10, static_cast<Word>(value));
            halted_ = false;
            break;
        }
        case ecall::kReadstring: {
            Word addr = ReadReg(10);
            Word max_len = ReadReg(11);
            Word count = 0; 
            char value;
            while (count < max_len - 1) {
                ...
                if(value == '\n')
                break;
                WriteMemoryByte(addr + count, value);
                count = count + 1;
            }
            WriteMemoryByte(addr + count, 0);
            halted_ = false;
            break;
        }
