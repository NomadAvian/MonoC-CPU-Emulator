...
Follow the [README.md#build](../README.md#build) for instructions. AI usage is totally optional. If you have `ollama` installed its detected automatically, but make sure the set model is the right one in the `.env` file.
...
> **Note:** By default, Ollama strictly blocks all traffic that does NOT originate from `127.0.0.1`. Thats why ollama must be started with `env OLLAMA_HOST="0.0.0.0" ollama serve`.
